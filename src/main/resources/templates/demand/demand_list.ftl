...
<html>
	...
	<body>
		<div class="admin-main">
            <form id="form" action="/admin/demand_list" method="post" >
			<blockquote class="layui-elem-quote">
		        ...
		        <select name="demandStatus" id="demandStatus">
					  ...
					  <option value="0"  <#if demandStatus??><#if demandStatus=='0'>selected</#if></#if>>待响应</option>
					  <option value="1"  <#if demandStatus??><#if demandStatus=='1'>selected</#if></#if>>需求确认中</option>
					  <option value="2"  <#if demandStatus??><#if demandStatus=='2'>selected</#if></#if>>订单确认</option>
					  <option value="3"  <#if demandStatus??><#if demandStatus=='3'>selected</#if></#if>>订单进行</option>
					  <option value="4"  <#if demandStatus??><#if demandStatus=='4'>selected</#if></#if>>正在配货</option>
					  <option value="5"  <#if demandStatus??><#if demandStatus=='5'>selected</#if></#if>>等待付款</option>
					  <option value="6"  <#if demandStatus??><#if demandStatus=='6'>selected</#if></#if>>已完成</option>
					  <option value="7"  <#if demandStatus??><#if demandStatus=='7'>selected</#if></#if>>终止申请中</option>
					  <option value="8"  <#if demandStatus??><#if demandStatus=='8'>selected</#if></#if>>已终止</option>
                </select>
                </div>
                ...
                <select id="categoryId" name="categoryId">
                  <option value="">全部</option>
                  <#list list as ca>
                      <option value="${ca.id}" <#if categoryId??><#if categoryId==ca.id>selected</#if></#if> >${ca.categoryName}</option>
                  </#list>
                </select>
                </div>
                                       发布时间：
		        <div class="layui-input-inline">
                   <input class="layui-input" placeholder="开始区间" id="LAY_demorange_s" name="publishedTimeStart" value="${(publishedTimeStart)!''}">
                </div>
			    <div class="layui-input-inline">
			      <input class="layui-input" placeholder="结束区间" id="LAY_demorange_e" name="publishedTimeEnd" value="${(publishedTimeEnd)!''}">
			    </div>
						...
						<#list page.list as demand>
						    <tr title="点击查看sku经理">
								...
								<td>
									...
									<a href="javascript:del('${demand.id}',$(this));" class="layui-btn layui-btn-mini" titie="删除">删除</a>
						 ...
						 $.ajax({
				           ...
				           success:function(data) { 
                            ...
                            $.each(data.list,function(i,val){
								...
								str +=' <a href="javascript:del(\''+val.id+'\',$(this));" class="layui-btn layui-btn-mini" titie="删除">删除</a>'
